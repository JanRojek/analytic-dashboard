...
public sealed class GetDatasetProfileEndpointTests
    : IClassFixture<ApiFixture>
{
    ...
    public GetDatasetProfileEndpointTests(
        ApiFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    [Fact]
    public async Task GetDatasetProfile_ShouldReturnProfileForImportedDataset()
    {
        ...
        try
        {
            ...
            var profile = await profileResponse.Content
                .ReadFromJsonAsync<DatasetProfileResponse>(
                    CancellationToken
                );
            ...
            var amountColumn = profile.Columns.Single(
                column =>
                    column.Name == "Amount"
            );
            ...
            Assert.Equal(
                20d,
                amountColumn.Avg.Value
            );

            Assert.Equal(
                3,
                profile.PreviewRows.Count
            );
        }
        finally
        {
    ...
    private static HttpRequestMessage CreateProfileRequest(
        Guid projectId,
        Guid datasetId,
        Guid userId)
    {
