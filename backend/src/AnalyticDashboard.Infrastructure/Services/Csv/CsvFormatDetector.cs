...
{
    public CsvFormatDetectionResult Detect(
        TextReader reader)
    {
        var sampleLines = ReadSampleLines(
            reader
        );
    ...
    private static List<char> GetDelimiterCandidates(
    ...
    private static CsvFormatDetectionResult DetectDelimiter(
        IReadOnlyList<string> lines)
    {
        var foundInconsistentCandidate = false;

        var candidates = GetDelimiterCandidates(
            lines
        );

        foreach (var candidate in candidates)
        {
            var delimiterCountsPerLine =
                new int[lines.Count];

            for (var i = 0; i < lines.Count; i++)
            {
                delimiterCountsPerLine[i] =
            ...
            var appearsInAnyLine =
                delimiterCountsPerLine.Any(
                    x => x > 0
                );

            var sameCountInEveryLine =
                delimiterCountsPerLine.All(
                    x => x == delimiterCountsPerLine[0]
                );

            if (appearsInAnyLine
                && !sameCountInEveryLine)
            {
